...
parser.add_argument('--attr_method', type=str, default='final')
...
if __name__ == "__main__":
    ...
    # if not os.path.exists(f"{perfix}/{args.model}_final{'_singlesoftmax' if args.single_softmax else ''}_scores.npz"):
    with torch.no_grad():
        ...
        attribution = np.load(f"final_attribution_isa/{args.model}_final_attribution{'_singlesoftmax' if args.single_softmax else ''}.npy")
        ...
        np.savez(f"{perfix}/{args.model}_final{'_singlesoftmax' if args.single_softmax else ''}_scores.npz", **scores)