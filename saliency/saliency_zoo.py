from saliency.core import FastIG,FastIGKL, GuidedIG,negflux_pgd_step,pgd_step_gai2,pgd_step_loss_balance, pgd_step,pgd_step_kl,pgd_step_gai, BIG, FGSM,FGSMKL, MFABA, SaliencyGradient,SmoothGradient,DL,FGSMGrad,IntegratedGradient,FGSMGradALPHA,dct_2d,idct_2d,DI,gkern,exp,AMPE,FGSMGradSSA
...
def negflux(model, data,target, epsilon=0.1, max_iter=10, topk=20):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
    random.seed(3407)
...
def big(model, data, target, data_min=0, data_max=1, epsilons=[36, 64, 0.3 * 255, 0.5 * 255, 0.7 * 255, 0.9 * 255, 1.1 * 255], class_num=1000, gradient_steps=2):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
...
def ig(model, data, target, gradient_steps=25):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
...
def sg(model, data, target,stdevs=0.15, gradient_steps=25):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
...
def isa(model,x,label,step_size=10000,add_steps=2,minus_steps=2,alpha=0.004,factor=1.3):
...
def ampe(model, data, target, data_min=0, data_max=1, epsilon=16,N=5,num_steps=5, use_sign=True, use_softmax=True):
    assert len(data.shape) == 4, "Input data must be 4D tensor"
    mfaba = AMPE(model)
    epsilon = epsilon / 255
    attack = FGSMGradSSA(
