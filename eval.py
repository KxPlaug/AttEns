...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{perfix}/{args.model}_{args.attr_method}{'_singlesoftmax' if args.single_softmax else ''}_scores.npz"):
        with torch.no_grad():
            ...
            deletion = CausalMetric(model, 'del', 448, substrate_fn=substate_fn if args.eval_method == "now_gai" else torch.zeros_like,reverse=False)
            insertion = CausalMetric(model, 'ins', 448, substrate_fn=substate_fn if args.eval_method == "now_gai" else torch.zeros_like,reverse=False)
